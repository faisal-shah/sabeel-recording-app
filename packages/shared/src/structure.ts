 ...
 * on the class document (`CourseDoc.effectiveActive`) whenever either side
 ...
 *
 * Note what it implies: archiving a cohort deactivates its courses without
 ...
 * because the cascade is derived rather than written into the courses.
 */
export function deriveEffectiveActive(
  cohortArchived: boolean,
  courseArchived: boolean,
): boolean {
  return !cohortArchived && !courseArchived;
}

/**
 * May a student play recordings from this class right now?
 *
 * Active courses: yes. Archived ones: only if staff explicitly kept access on —
 ...
 */
export function canPlayFromCourse(cls: {
